...
async def call_voice_agent(request : Request):
    try:
        ...
        call = client.calls.create(
            ...
            # url="https://1b81-2401-4900-1c17-8ec9-556e-70cc-2c10-86d9.ngrok-free.app/webhook"
            url=f"https://abee0e3b867d.ngrok-free.app/incoming-call?{query_string}"
        )
...
async def media_stream_websocket(websocket: WebSocket):
    ...
    ### Flow:
    1. Greet: "Hello, am I speaking with {global_context['candidate']}"
    2. If yes, proceed to verification
    3. Verify: "We received your application for {global_context['role']} at {global_context['company']}. Is now a good time for a quick 2-3 minute screening?"
    ...
    try:
        ...
        if isinstance(session['transcript'], list):
            ...
            transcript_text = session['transcript'].replace('\\n', '\n')
            ...
            async with session.post(
                ...
                json={
                    ...
                    "messages": [
                        {
                            "role": "system",
                            "content": "You are a recruiting assistant. Extract candidate information from the conversation transcript and return it as a JSON object with the following structure: {\"candidateName\": \"string\", \"candidatePhone\": \"string\", \"desiredRole\": \"string\", \"experienceYears\": \"string\", \"skills\": [\"skill1\", \"skill2\"], \"availability\": \"string\", \"specialNotes\": \"string\"}. Only return the JSON object, no additional text."
                        },
                        {
                            "role": "user",
                            "content": transcript
                        ...
                        "json_schema": {
                            ...
                            "schema": {
                                ...
                                "properties": {
                                    ...
                                    "candidatePhone": {"type": "string", "description": "Phone number of the candidate"},
                                    "desiredRole": {"type": "string", "description": "The job role or position candidate is applying for"},
                                    "experienceYears": {"type": "string", "description": "Years of work experience"},
                                    "skills": {
                                        "type": "array",
                                        "items": {"type": "string"},
                                        "description": "List of key skills mentioned"
                                    },
                                    "availability": {"type": "string", "description": "Candidate's availability for joining or interview"},
                                    "specialNotes": {"type": "string", "description": "Any additional notes or preferences mentioned"}
                                },
                                "required": ["candidateName", "desiredRole", "availability"]
                            }
                        }
                    }
                }
        ...
        if (result.get('choices') and 
            ...
            try:
                ...
                fallback_data = {
                    ...
                    "specialNotes": f"Raw transcript analysis: {content}"
                }
